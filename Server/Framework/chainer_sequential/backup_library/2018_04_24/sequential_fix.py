...
import inspect
from chainer_ext.functions_view import entropy
    ...
    def predict_with_mask(self, x, ent_Ts=None, test=True):
        ...
        for i, link in enumerate(self.links):
            if isinstance(link, Sequential):
                b = link(x, test=test)
                b = b[0]
                #print "x : %s \b b : %s \b ent_Ts : %s " % (x.data.shape,b.data,ent_Ts[min(exit_i,len(ent_Ts)-1)])
