...
static void blinear_layer(const uint8_t* A, const uint8_t* F, uint8_t* C,
	...
	const int k, const int w, const int h, const float ent_T, const int model, uint8_t* bA);
static void fconv_layer(const float* A, const uint8_t* F, uint8_t* C,
	...
	const int pl_pw, const int pl_ph, uint8_t* bC);
...
static void fconv(const float* A, const uint8_t* F, uint8_t* C,
	...
	const int pl_sh, const int pl_pw, const int pl_ph, uint8_t* bC);
...
static void blinear_layer(const uint8_t* A, const uint8_t* F, uint8_t* C,
	...
	const int k, const int w, const int h, const float ent_T, const int model, uint8_t* bA)
{
	int i, j, ni, ri, ci, max_idx, f , C_size;
	float res, max_res;
        f = n / (w*h);
	C_size = n / 8;
	...
	//printf("---------------blinear layer---------------- \n");

	float* classify = malloc(sizeof(float)*k);
	...
	for (i = 0; i < m; ++i) {
		...
		C[i] = max_idx;
                ...
                ent = entropy(classify, k);
		gettimeofday(&e_t, NULL);
		if (ent > ent_T)
			pass_cloud(model, bA, layer_number, m, n, w, h);
	}
	...
	const int pl_pw, const int pl_ph, uint8_t* bC)
{
	...
	if(OpenCL_enable == 1)
		fconv_ocl(A,F,C,Bias,Gamma,Beta,Mean,Std,m,num_f,w,h,d,kw,kh,sw,sh,pw,ph,pl_w,pl_h,pl_sw,pl_sh,pl_pw,pl_ph,bC,max_m);
	else{
		for (i = 0; i < m; ++i) {
			for (j = 0; j < num_f; ++j) {
				a_idx = i*w*h*d;
	...
	const int pl_pw, const int pl_ph, uint8_t* bC)
{
	...
	if(OpenCL_enable == 1)
		bconv_ocl(A,F,C,Bias,Gamma,Beta,Mean,Std,m,num_f,w,h,d,kw,kh,sw,sh,pw,ph,pl_w,pl_h,pl_sw,pl_sh,pl_pw,pl_ph,bC,max_m);
	else{
		for (i = 0; i < m; ++i) {
			for (j = 0; j < num_f; ++j) {
